...
string Database::getUserName(const int& userId) const
{
...
void Database::dellUser(std::string username)
{
    _usersMapByName.erase(username);
    if(_users.size() == 1 || _users.rbegin()->getuserName() == username){
        _users.pop_back();
    }
    else {
        bool runflag = false;
        for (std::size_t i = 1; i < _users.size(); ++i){
            if(runflag == true){
               _users[i]= _users[i-1];
            }else{
                if ( _users[i].getuserName() == username) runflag = true;
            }
